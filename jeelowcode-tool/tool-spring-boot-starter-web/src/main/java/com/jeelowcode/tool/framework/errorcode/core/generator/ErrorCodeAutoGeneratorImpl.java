...
public class ErrorCodeAutoGeneratorImpl implements ErrorCodeAutoGenerator {
     ...
     * 错误码 Api
     */
//    private final IApiErrorCodeApi apiErrorCodeApi;
    ...
    public void execute() {
       /* // 第一步，解析错误码
        ...
        // 第二步，写入到 system 服务
        try {
            apiErrorCodeApi.autoGenerateErrorCodeList(autoGenerateDTOs);
    ...
    private List<ErrorCodeAutoGenerateReqDTO> parseErrorCode() {
        ...
        if (CollUtil.isEmpty(constantsClassList)) {
            log.info("[execute][未配置 jeelowcode.error-code.constants-class-list 配置项，不进行自动写入到 system 服务中]");
