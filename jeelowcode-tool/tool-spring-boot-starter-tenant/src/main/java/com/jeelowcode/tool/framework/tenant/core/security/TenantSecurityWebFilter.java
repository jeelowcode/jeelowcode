package com.jeelowcode.tool.framework.tenant.core.security;
...
import com.jeelowcode.tool.framework.web.core.filter.ApiRequestFilter;
...
public class TenantSecurityWebFilter extends ApiRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (!isIgnoreUrl(request)) {
            ...
            if(!isJeeLowCodeUrl(request)){//低代码平台有自己一套租户逻辑
    ...
    private boolean isJeeLowCodeUrl(HttpServletRequest request) {
        List<String> jeeLowCodeUrlList=new ArrayList<>();
        jeeLowCodeUrlList.add(JeeLowCodeBaseConstant.REQUEST_URL_START+"/** ");
        // 快速匹配，保证性能
        if (CollUtil.contains(jeeLowCodeUrlList, request.getRequestURI())) {
            return true;
        }
        // 逐个 Ant 路径匹配
