...
import com.jeelowcode.service.system.entity.RoleMenuDO;
import com.jeelowcode.service.system.entity.UserRoleDO;
import com.jeelowcode.service.system.mapper.RoleMenuMapper;
import com.jeelowcode.service.system.mapper.UserRoleMapper;
import com.jeelowcode.service.system.config.redis.RedisKeyConstants;
import com.jeelowcode.service.system.enums.DataScopeEnum;
import com.jeelowcode.service.system.service.IDeptService;
import com.jeelowcode.service.system.service.IJeeLowCodePermissionService;
...
public class PermissionServiceImpl implements IPermissionService {
    ...
    @Autowired
    private IJeeLowCodePermissionService jeeLowCodePermissionService;
    ...
    public boolean hasAnyPermissions(Long userId, String... permissions) {
        ...
        for (String permissionListStr : permissions) {
            List<String> permission2List = jeeLowCodePermissionService.permission2List(permissionListStr);
            boolean permissionFlag=true;
            for(String permission:permission2List){
                boolean jeeLowCodeFlag = jeeLowCodePermissionService.hasAnyPermission(userId, roleIds,permission);
                if(jeeLowCodeFlag){//里面校验通过，则直接说明有权限
                    continue;
                }
                ...
                permissionFlag=false;//有一个没有权限的话 = 所有没有权限
            }
            if(permissionFlag){
    ...
    @Override
    @Caching(evict = {
            @CacheEvict(value = RedisKeyConstants.PERMISSION_MENU_ID_LIST, allEntries = true), // all
            @CacheEvict(value = RedisKeyConstants.MENU_ROLE_ID_LIST,allEntries = true), // all
            @CacheEvict(value = RedisKeyConstants.USER_ROLE_ID_LIST, key = "#userId")
    })
